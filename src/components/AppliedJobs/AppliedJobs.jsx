...
const AppliedJobs = () => {
    ...
    return (
        <div>
            <h2 className="text-2xl font-bold text-center">Jobs I applied: {appliedJobs.length}</h2>
            ...
            <ul>
                {
                    appliedJobs.map(job => <li key={job.id}>
                        <span>{job.job_title} at {job.company_name}: {job.remote_or_onsite}</span>
                    </li>)
                }
            </ul>
        </div>
